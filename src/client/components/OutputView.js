...
function OutputView({ status, testsResults, classes }) {
  const passedTests = testsResults
    ? testsResults.tests.filter((test) => test.passed)
    : null;
  const failedTests = testsResults
    ? testsResults.tests.filter((test) => !test.passed)
    : null;
    ...
    return (
      <Paper elevation={1} square className={classes.container}>
        <div className={classes.noTestsResults}>
          ...
          <Typography component="p" className={classes.typography}>
            An error ocurred when making the evaluation of the tests, please try
            again.
          </Typography>
        </div>
      </Paper>
      ...
      <List
        ...
        subheader={
          <ListSubheader component="div">
            {`Output from tests. Execution time: ${
              testsResults.executionTime
            }ms`}
          </ListSubheader>
        }>
        <ExpandableListItem
